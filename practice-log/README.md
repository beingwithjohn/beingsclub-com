...
day. The canonical Practice Log now lives at `https://spacetobe.xyz/log/`; the
old Beings Club addresses are noindex compatibility move pages only. It has no
participant directory. The host can grant the private line to
...
practice-log/
  ...
  app/
    ...
    host.js                  John's page
    shell.html               archived Beings Club frontend source
    build.js                 verifies the retired move pages; writes nothing
  seed/seed.js               make a run and the people in it
  dev/serve.js  dev/emails.js
  test/                      unit tests and the §7 checks
```

`build/build_shell.py` owns the old `/log/**` addresses and emits move pages to
Space to Be. The frontend source here is retained as migration history only;
its build command refuses to recreate the retired Beings Club application.
...
# evergreen, joinable by anyone from https://spacetobe.xyz/log/
...
The canonical frontend is published from the Space to Be repository. Deploying
this repository publishes only the compatibility moves and the shared Worker.
...
npm run check                  # retired-route and committed-secret checks
node app/build.js              # verifies the move pages; writes nothing
...
`npm run check` verifies that both old Beings Club addresses are small noindex
move pages to Space to Be, that the retired build cannot restore the old app,
and that nothing secret-looking is committed in the shared Worker.
...
**Migration.** Nothing in the Beings Club marketing shell links to the log.
Old `beingsclub.com/log/` bookmarks move immediately to the canonical Space to
Be page, and new Practice Log email links return there directly.
