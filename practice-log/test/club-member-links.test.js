...
test('each Salon email entrance is one-use without invalidating another entrance', async () => {
  ...
  const messages = await issueMemberAccessLink(env, 9, timestamp + 3, 'messages');
  ...
  assert.match(messages, /^https:\/\/beingsclub\.com\/members\/#welcome=[A-Za-z0-9_-]+&next=messages$/);
  assert.equal(statements.filter(({ sql }) => sql.includes('INSERT INTO member_welcome_link')).length, 4);
