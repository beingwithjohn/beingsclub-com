...
(function () {
  ...
  function render() {
    ...
    if (S.run.phase === 'gathering' && view !== 'settings' && view !== 'john') {
      return viewGathering();
    }
  ...
  // -------------------------------------------------------------------------
  // the gathering — after taking a place, before day one
  // -------------------------------------------------------------------------
  function viewGathering() {
    ...
    inner.appendChild(h('<div class="band"><div class="caps" style="margin-bottom:8px;">Your place is held</div>' +
      '<p>' + esc(startsLine()) + '</p></div>'));
    ...
    // Who is here. The point of the gathering.
    var list = h('<div style="display:grid;gap:16px;"><div class="caps">Who’s here' +
      (ros.places_left != null ? ' · ' + esc(placesLine(ros.places_left).replace(/\.$/, '')) : '') + '</div>' +
    ...
    shell(inner, {
      right: '<span class="barlab">' + esc(r.name) + '</span>',
      above: answerStrip(),
      foot: foot
    });
  ...
  function viewContribute() {
    var suggested = S.run.suggested_amount;
    ...
    var inner = h('<div class="pad narrow" style="flex:1;display:flex;flex-direction:column;' +
      ...
      (suggested ? '<div class="frame"><span class="caps">Suggested</span>' +
        '<p class="body">' + sym + (suggested / 100).toFixed(0) + '. Type over it with anything ' +
        'you like on the next page.</p></div>' : '') +
